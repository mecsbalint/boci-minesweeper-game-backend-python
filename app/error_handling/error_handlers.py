...
def init_error_handlers(app: Flask):
    ...
    def handle_validation_error(exception: ValidationError):  # pyright: ignore[reportUnusedFunction]
        ...
        for error in errors:
            error_logs.append(ExceptionDto(code=error["type"], message=f"{"->".join(map(str, error["loc"]))}: {error["msg"]}"))
...
def http_exception_handler(exception: HTTPException):
    ...
    return jsonify(ExceptionDto(code=f"HTTP_{type(exception).__name__}", message=description)), response_code
