...
def init_error_handlers(app: Flask):
    ...
    def handle_database_connection_error(exception: OperationalError | InterfaceError):  # pyright: ignore[reportUnusedFunction]
        return jsonify([ErrorDetailDto(code="DATABASE_CONNECTION_FAILED", message=str(exception.__cause__)).model_dump()]), 500
    ...
    def handle_database_query_error(exception: IntegrityError | DataError | ProgrammingError):  # pyright: ignore[reportUnusedFunction]
        return jsonify([ErrorDetailDto(code="DATABASE_QUERY_FAILED", message=str(exception.__cause__)).model_dump()]), 500

    @app.errorhandler(SQLAlchemyError)
    def handle_database_error(exception: SQLAlchemyError):  # pyright: ignore[reportUnusedFunction]
        return jsonify([ErrorDetailDto(code="DATABASE_ERROR", message=str(exception.__cause__)).model_dump()]), 500
...
def http_exception_handler(exception: HTTPException):
    ...
    return (
        jsonify([ErrorDetailDto(code=f"HTTP_{type(exception).__name__}", message=description).model_dump()]),
        response_code
        )
