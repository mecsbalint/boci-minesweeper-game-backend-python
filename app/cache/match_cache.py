...
KeyType = Literal["user", "match", "match_exp_key"]


@handle_cache_errors
def save_match_to_cache(match: Match, type: SaveType) -> Match:
    user_keys = [get_key(type, "user", participant.user_id) for participant in match.participants]

    match.id = match.id if match.id else uuid4()

    match_key = get_key(type, "match", match.id)

    match_exp_key = get_key(type, "match_exp_key", match.id)

    with redis.pipeline() as pipeline:  # pyright: ignore[reportUnknownMemberType]
        pipeline.watch(match_key, match_exp_key, *user_keys)
        ...
        pipeline.multi()
        match.version += 1
        pipeline.set(match_key, pickle.dumps(match))
        pipeline.set(match_exp_key, match_key, ex=REDIS_TIMEOUT)
...
def get_match_by_user_id_from_cache(user_id: int, type: SaveType) -> Match:
    user_key = get_key(type, "user", user_id)
...
def get_match_by_id_from_cache(match_id: UUID, type: SaveType) -> Match:
    match_key = get_key(type, "match", match_id)
    match_exp_key = get_key(type, "match_exp_key", match_id)
    match_bytes = cast(bytes | None, redis.get(match_key))
    redis.expire(match_exp_key, REDIS_TIMEOUT)
...
def check_match_in_cache(user_id: int, type: SaveType) -> bool:
    ...
    match_id = UUID(match_id_str)
    match_exp_key = get_key(type, "match_exp_key", match_id)
    is_exist = redis.exists(match_exp_key) == 1
    if is_exist:
        redis.expire(match_exp_key, REDIS_TIMEOUT)
    return is_exist


def get_key(type: SaveType, key_type: KeyType, id: int | UUID) -> str:
    return f"{type}_{key_type}_{id}"


def get_match_key_from_match_exp_key(match_exp_key: str) -> str:
    return match_exp_key.replace("match_exp_key", "match")


def get_type_from_match_key(match_key: str) -> SaveType:
    return cast(SaveType, match_key.split("_")[0])
