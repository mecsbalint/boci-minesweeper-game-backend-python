...
from app.error_handling.exceptions import InvalidUserException, UserNotFoundException
from app.service.user_service import get_user_by_id


def add_message(user_id: int, message: str) -> tuple[UUID, list[tuple[str, str]]]:
    match = get_match_by_user_id_from_cache(user_id, "MP")
    user = get_user_by_id(user_id)

    if not user:
        raise UserNotFoundException("id")

    if user_id not in [participant.user_id for participant in match.participants]:
